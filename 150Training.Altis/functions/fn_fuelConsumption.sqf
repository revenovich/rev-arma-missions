/*
	...
	[_this, rateOfLeak] spawn OFT_fnc_fuelConsumption;

	When calling from the debug console:
	[nameOfVehicle, rateOfLeak] spawn OFT_fnc_fuelConsumption;
...
fn_fuelDecrease = {
	...
	_vehicle setVariable ["OFT_fuel_scriptRunning", true, true];
	...
	while {alive _vehicle} do {
		...
		if (!local _vehicle) exitWith {
			_vehicle setVariable ["OFT_fuel_scriptRunning", false, true];
		};
...
while {alive _vehicle} do {
	...
	waitUntil {isNull _vehicle or !(alive _vehicle ) or !(_vehicle getVariable ["OFT_fuel_scriptRunning", false])};
};
