...
DLM_fnc_spawn2_waypointBehaviour = 
{
...
DLM_fnc_spawn2_randomPatrol = 
{
	...
	{
		_pos = [_center,(random _radius),(random 360)] call DLM_fnc_radPos;
		_wp = _group addWaypoint [_pos,0];
	...
	_group spawn DLM_fnc_spawn2_waypointBehaviour;
};

DLM_fnc_spawn2_perimeterPatrol = 
{
	if(!isServer) exitWith{};
	...
	for [{_x=1},{_x<=_waypointNumbers},{_x=_x+1}] do
	{
		_pos = [_center,_radius,_currentAngle] call DLM_fnc_radPos;
		_wp = _group addWaypoint [_pos,0];
		_wp setWaypointType "MOVE";
		_wp setWaypointSpeed "LIMITED";
		_wp setWaypointFormation "STAG COLUMN";
...
DLM_serverRespawn = 
{
	...
	private ["_x","_y"];
	{
		...
		_x setPos [0,0,0];
		_x setDamage 1;
	}forEach ((getPos trg_DLM_admin) nearEntities [["Air","Car","Motorcycle","Tank"],5000]);
};

DLM_serverSingleRespawn = 
{
