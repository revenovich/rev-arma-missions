...
_blackListedLoadouts = ["LOADOUT COMMANDER", "LOADOUT SECOND COMMANDER", "LOADOUT SNIPER", "LOADOUT SPOTTER", "LOADOUT UAV OPERATOR", "LOADOUT AUTORIFLEMAN", "LOADOUT AT RIFLEMAN", "LOADOUT TEAMLEADER"];
...
if (_isUseUnit) then {
	...
	// AT Specialist
	if (isNil "oft_atspecialist") then {
		_nullUnits pushBack "AT Specialist";
	} else {
		[5, oft_atspecialist, "LOADOUT AT SPECIALIST"] call OFT_fnc_gearHandle;
		_allAvailableLoadoutNames pushBack "LOADOUT AT SPECIALIST";
		_notNullUnits pushBack oft_atspecialist;
	};
	...
	// Uav Operator
	if (isNil "oft_uavoperator") then {
		_nullUnits pushBack "Uav Operator";
	} else {
		[5, oft_uavoperator, "LOADOUT UAV OPERATOR"] call OFT_fnc_gearHandle;
		_allAvailableLoadoutNames pushBack "LOADOUT UAV OPERATOR";
		_notNullUnits pushBack oft_uavoperator;
	};
