...
fn_smoothSetPos = {
    ...
    [_object, _startPos, _endPos, _time] spawn {
        ...
        while {time < _endTime} do {
            ...
            _object setPosATL _currentPos;
			_object setVectorDirAndUp [_currentDir, _currentUp];
            sleep 0.01;
        };
        
        _object setPosATL _endPos;
		_object setVectorDirAndUp [_currentDir, _currentUp];
    };
};
...
[_object, _maxZ, _minZ, _maxUpdateTime, _minUpdateTime] spawn {
    ...
    waitUntil {missionNamespace getVariable ["antiGravity", false]};

    private _currentZ = getPosATL _object select 2;
    ...
    while {true} do {
        ...
        [_object, [getPosATL _object select 0, getPosATL _object select 1, _targetZ], _updateTime] call fn_smoothSetPos;
